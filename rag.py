...
def add_text_file(text_path, source_name=None):
    """
    Indexa un archivo de texto plano (.md, .txt) en Pinecone.
    Hace chunking del texto y genera embeddings con Gemini.
    Retorna (chunk_count, mensaje).
    """
    index = get_index()
    if index is None:
    ...
    try:
        ...
        if not text or len(text.strip()) < 100:
            return 0, "El archivo está vacío o es demasiado corto."
        ...
        logger.error(f"Error procesando archivo de texto: {e}", exc_info=True)
